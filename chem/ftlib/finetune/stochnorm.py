    ...
    def forward(self, input):
        ...
        if self.training:
            z_0 = F.batch_norm(
                ...
                False, self.momentum, self.eps) # trainning=False, track_running_stats=True。这个是期望中的测试阶段的设置，此时BN会用之前训练好的模型中的（假设已经保存下了）running_mean和running_var并且不会对其进行更新。一般来说，只需要设置model.eval()其中model中含有BN层，即可实现这个功能

            z_1 = F.batch_norm(
                input, self.running_mean, self.running_var, self.weight, self.bias,
                True, self.momentum, self.eps) # trainning=True, track_running_stats=True。这个是期望中的训练阶段的设置，此时BN将会跟踪整个训练过程中batch的统计特性。
...
class StochNorm1d(_StochNorm):
    ...
    """
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True, p=0.5):
        super(StochNorm1d, self).__init__(num_features, eps=eps, momentum=momentum, affine=affine,
                                         track_running_stats=track_running_stats, p=p)
        pass
    def _check_input_dim(self, input):
...
def convert_model(module, p):
    ...
    for pth_module, stoch_module in zip([torch.nn.modules.batchnorm.BatchNorm1d,
        ...
        if isinstance(module, pth_module):
            mod = stoch_module(num_features=module.num_features, eps=module.eps, momentum=module.momentum, affine=module.affine,
                                         track_running_stats=True, p=p)
